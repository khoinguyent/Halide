...
class StorageAccountListView extends StatelessWidget {
                  ...
                  Expanded(
                    child: BlocBuilder<StorageAccountsBloc, StorageAccountsState>(
                      builder: (context, state) {
                        ...
                        } else if (state is StorageAccountsError) {
                          return Center(child: Text(state.message, style: const TextStyle(color: Colors.red)));
                        } else if (state is StorageAccountsLoaded) {
                          return ListView.separated(
                            itemCount: state.accounts.length,
                            separatorBuilder: (context, index) => const SizedBox(height: 16),
                            itemBuilder: (context, index) {
                              final account = state.accounts[index];
                              return _StorageAccountCard(account: account);
                            },
                          );
                        }
